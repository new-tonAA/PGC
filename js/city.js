...
class CitySystem {
    ...
    generate(terrain, seed, vehicleCount, buildingDensity) {
        ...
        if (typeof buildingDensity === 'object' && buildingDensity !== null) {
            bldDensity = buildingDensity.buildingDensity || buildingDensity.buildingCount || 8;
    ...
    generateCoastalCity(terrain, vehicleCount) {
        ...
        for (const pos of roadPositions) {
            for (let t = -halfSize; t <= halfSize; t += (this.lightSpacing || 14)) {
    ...
    regenerateVehicles(terrain, vehicleCount) {
        ...
        if (terrain.terrainType === 'islands') {
            ...
            const ps = (terrain.terrainType === 'suburban') ? 10 : 8;
            const hs = terrain.size * 0.4;
            const pp = [];
            for (let p = -hs; p <= hs; p += ps) pp.push(p);
            this.generateVehicles(terrain, pp, hs, vehicleCount, terrain.terrainType === 'city');
        }
    }
    ...
    updateSnowAccum(s, dt) { /* snow handled by terrain shader */ }
}
